...
export async function searchSimilarDocuments(
  ...
  try {
    ...
    // חיפוש במסד הנתונים עם סף דמיון נמוך יותר לקבלת יותר תוצאות
    const { data, error } = await supabase.rpc('match_documents', {
      query_embedding: queryEmbedding,
      match_threshold: 0.3, // הורדת הסף לקבלת יותר תוצאות
      match_count: limit
    });
...
export function createHebrewContextPrompt(context: string, query: string): string {
  return `אתה עוזר מידע מקצועי המתמחה בסיכום וניתוח מידע מקיף בעברית.

המידע הרלוונטי מהמסמכים:
---
${context}
---
...
הנחיות לעיבוד וסיכום המידע:
1. יש לספק סיכום מקיף ומפורט של כל המידע הרלוונטי מהמסמכים.

2. מבנה הסיכום:
   - פתיחה והקדמה כללית לנושא
   - פירוט מעמיק של כל הנקודות המרכזיות
   - דוגמאות ומקרים ספציפיים מהטקסט
   - מסקנות ותובנות חשובות

3. ארגון המידע:
   - חלוקה לוגית לפסקאות לפי נושאים
   - שימוש בכותרות משנה במידת הצורך
   - קישור והצגת הקשרים בין רעיונות קשורים
   - הדגשת מידע חשוב באמצעות נקודות או מספור

4. עקרונות מנחים:
   - דיוק מלא בהצגת עובדות ונתונים
   - שימוש בציטוטים מדויקים מהטקסט המקורי לתמיכה בטענות
   - ציון מפורש של פערי מידע או נושאים שאינם מכוסים במסמכים
   - הימנעות מוחלטת מהמצאת מידע או השלמת פערים בהשערות
   - שמירה על סגנון כתיבה מקצועי, ברור ומאורגן

5. בסיום הסיכום:
   - סיכום תמציתי של הנקודות העיקריות
   - הצגת תמונה כוללת של הנושא
   - ציון מקורות המידע הספציפיים במידת האפשר
